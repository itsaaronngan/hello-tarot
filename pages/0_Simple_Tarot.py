...
def generate_tarot_reading(tarot_draw, deck, language, context, draw_style, num_cards):
    ...
    Avoid white juju and overly positive language that is overly vague/shallow. Avoid self-help buzzwords. subtly and gently give a reading suitable for someone who has been broken up with and promote getting complete rest and leaving the past in the past (but do not mention this as part of the 70 word introduction, and do not mention the breakup as a part of the stated context, subtle is the key). Instead of using words like "transformation" use "transmutation", instead of "manifest" make references to "taking action" Use a gentle, empathetic, and respectful tone. Avoid overly formal language. If appropriate, reference the complexity inherant in life: e.g. happiness cannot be appreciated without sorrow.
